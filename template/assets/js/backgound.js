...
function init_backgrounds() {
	...
	if (
		...
		option_hero_background_mode_mobile != "match"
	) {
...
function colorBackground() {
	$("body").append(
		'<div class="bg-color" style="background-color:' +
			option_hero_background_color_bg +
			'"></div>'
	);
}
...
function squareBackground() {
	$("body").append(
		'<div class="bg-color" style="background-color:' +
			option_hero_background_square_bg +
			'"></div>'
	);
	$("#main").append(
		'<ul class="bg-bubbles ' +
			option_hero_background_square_mode +
...
function asteroidsBackground() {
	...
	scene.fog = new THREE.Fog(
		option_hero_background_asteroids_bg_color,
		2.5,
		3.5
	);
	...
	var light = new THREE.SpotLight(
		option_hero_background_asteroids_spotlight_color,
		option_hero_background_asteroids_spotlight_intensity
	);
	...
	var lightBack = new THREE.PointLight(
		option_hero_background_asteroids_pointlight_color,
		option_hero_background_asteroids_pointlight_intensity
	);
	lightBack.position.set(0, -3, -1);

	var rectLight = new THREE.RectAreaLight(
		option_hero_background_asteroids_rectarealight_color,
		option_hero_background_asteroids_rectarealight_intensity,
		2,
		2
	);
	...
	$("#canvas-asteroids").css(
		"opacity",
		option_hero_background_asteroids_scene_opacity
	);
	$("body").append(
		'<div class="bg-color" style="background-color:' +
			option_hero_background_asteroids_bg_color +
...
function circleBackground() {
	...
	$("#canvas-circle").css(
		"opacity",
		option_hero_background_circle_scene_opacity
	);
	$("body").append(
		'<div class="bg-color" style="background-color:' +
			option_hero_background_circle_bg_color +
			'"></div>'
	);
...
function linesBackground() {
	...
	$("body").append(
		'<div class="bg-color" style="background-color:' +
			option_hero_background_lines_bg_color +
			'"></div>'
	);
			...
			line.points.forEach(function (point, h) {
				(point.x = point.nx),
					(point.y =
						point.ny +
						Math.sin(
							(point.x * line.ran + (step + point.ny)) / 40
						) *
...
function twistedBackground() {
	...
	$("#canvas-twisted").css(
		"opacity",
		option_hero_background_twisted_scene_opacity
	);
	$("#canvas-twisted").css(
		"transform",
		"translateX(" + option_hero_background_twisted_x_offset + ")"
	);
	$("body").append(
		'<div class="bg-color" style="background-color:' +
			option_hero_background_twisted_bg_color +
			'"></div>'
	);
	...
	function resize() {
		...
		for (var i = 0; i < quality; i++) {
			layers.push({
				x:
					window.innerWidth / 1 +
					Math.sin((i / quality) * 2 * Math.PI) *
						(radius - layerSize),
				y:
					window.innerHeight / 2 +
					Math.cos((i / quality) * 2 * Math.PI) *
						(radius - layerSize),
				r: (i / quality) * Math.PI,
			});
		}
